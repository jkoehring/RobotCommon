...
public class FixedI2C extends SensorBase {
     ...
     */
    public FixedI2C(I2C.Port port, int deviceAddress) {
	...
	public synchronized boolean write(int registerAddress, int data) {
		...
		byte r = I2CJNI.i2CWrite((byte)m_port.getValue(), (byte) m_deviceAddress, dataToSendBuffer, (byte)buffer.length);
		return r < 0;
	}
	...
	public synchronized boolean writeBulk(byte[] data) {
		...
		byte r = I2CJNI.i2CWrite((byte)m_port.getValue(), (byte) m_deviceAddress, dataToSendBuffer, (byte)data.length);
		return r < 0;
	}
